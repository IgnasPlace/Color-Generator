...
const ColorContainer = ({ color }) => {
  ...
  const handleDragOver = (e) => {
    e.preventDefault();
    if(e.target.textContent.length > 0) {
      dispatch({type: 'setSecondSwapColor', payload: e.target.textContent.slice(0, 7)})
    }
  }
  const handleDrop = (e) => {
    e.preventDefault();
    const colorOneIdx = newColorArr.findIndex(color => color === store.firstSwapColor);
      const colorTwoIdx =  newColorArr.findIndex(color => color === store.secondSwapColor);
